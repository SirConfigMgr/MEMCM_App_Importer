<#	
	...
	===========================================================================
	 ...
	 Filename:     	MECM_App_Importer.ps1
	===========================================================================
	.DESCRIPTION
		Create MECM Applications (optional with PSADT).
...
Function Load-PkgInfo {
    ...
    ("System.Windows.Forms")
    $OpenFileDialog = New-Object System.Windows.Forms.OpenFileDialog
    $OpenFileDialog.filter = "MECM App Importer files (*.info)| *.info"
    $OpenFileDialog.ShowDialog()
    $FileFolder = Split-Path $OpenFileDialog.filename -Parent
    $FileName = Split-Path $OpenFileDialog.filename -Leaf
...
Function Create-Application {
    ...
    If ($ErrorAction) {
        ...
        $Window_MECM_Connection.IsOpen = $true
        $Label_MECM_ChildWindow.Content = "Cannot Create Application - View Log File"
    ...
    If ($CheckBox_PSADT.IsChecked = $true) {
        ...
        If ($ComboBox_Detection.Text -eq "Product Code") {
            ...
            If ($ErrorAction) {
                Write-Log -LogPath $LogPath -Message ("-->" + $ErrorAction | Out-String) -Component "Create_Application" -Type Error
                $Window_MECM_Connection.IsOpen = $true
                $Label_MECM_ChildWindow.Content = "Cannot Create Deployment Type - View Log File"
                $Global:AppCreated = $false
                Return
                }
            Else {
...
# MECM Tab
$Button_ConnectMECM = $Form.Findname("Button_ConnectMECM")
...
$TextBox_ConnectMECM = $Form.Findname("TextBox_ConnectMECM")
...
# MECM Tab Childwindow
$Window_MECM_Connection = $Form.Findname("Window_MECM_Connection")
$Label_MECM_ChildWindow = $Form.Findname("Label_MECM_ChildWindow")
$Button_MECM_ChildWindow_Close = $Form.Findname("Button_MECM_ChildWindow_Close")

# MECM Tab ApplicationSettings Childwindow
$Window_MECM_ApplicationSettings = $Form.Findname("Window_MECM_ApplicationSettings")
...
# MECM Tab CollectionSettings Childwindow
$Window_MECM_CollectionSettings = $Form.Findname("Window_MECM_CollectionSettings")
...
# MECM Tab DistributionSettings Childwindow
$Window_MECM_DistributionSettings = $Form.Findname("Window_MECM_DistributionSettings")
...
# MECM Tab DeploymentSettings Childwindow
$Window_MECM_DeploymentSettings = $Form.Findname("Window_MECM_DeploymentSettings")
...
# MECM Tab
$Button_ConnectMECM.Add_Click({
    Connect-MECM -SiteCode $TextBox_Sitecode.Text -ProviderMachineName $TextBox_Managementpoint.Text -ErrorAction SilentlyContinue -ErrorVariable ErrorAction
    If ($ErrorAction) {
        Write-Log -LogPath $LogPath -Message ($ErrorAction | Out-String) -Component "MECM_Connect" -Type Error
        $TextBox_ConnectMECM.Background = "Red"
        $TextBox_ConnectMECM.Text = "Failed"
        $TextBox_ConnectMECM.TextAlignment = "Center"
        $Global:MECMConnected = $false
        }
    Else {
        $Info = "Connected To MECM"
        Write-Log -LogPath $LogPath -Message ($Info | Out-String) -Component "MECM_Connect" -Type Info
        $Button_ConnectMECM.IsEnabled = $false
        $TextBox_ConnectMECM.Background = "Green"
        $TextBox_ConnectMECM.Text = "Connected"
        $TextBox_ConnectMECM.TextAlignment = "Center"
        $Button_AppSettings.IsEnabled = $true
        $Global:MECMConnected = $true
        }
    })

$Button_AppSettings.Add_Click({
    $Window_MECM_ApplicationSettings.IsOpen = $true
    })

$Button_AS_ChildWindow_Close.Add_Click({
    $Window_MECM_ApplicationSettings.IsOpen = $false
    If (($ComboBox_InstallBehaviorType.Text -ne "") -and ($ComboBox_LogonRequirementType.Text -ne "") -and ($MECMConnected -eq $true)) {
...
$Button_CollectionSettings.Add_Click({
    $Window_MECM_CollectionSettings.IsOpen = $true
    })

$Button_CS_ChildWindow_Close.Add_Click({
    $Window_MECM_CollectionSettings.IsOpen = $false
...
$Button_DistributionSettings.Add_Click({
    $Window_MECM_DistributionSettings.IsOpen = $true
    })

$Button_DistS_ChildWindow_Close.Add_Click({
    $Window_MECM_DistributionSettings.IsOpen = $false
...
$Button_DeploymentSettings.Add_Click({
    $Window_MECM_DeploymentSettings.IsOpen = $true
    })

$Button_DeplS_ChildWindow_Close.Add_Click({
    $Window_MECM_DeploymentSettings.IsOpen = $false
...
$Button_MECM_ChildWindow_Close.Add_Click({
    $Window_MECM_Connection.IsOpen = $false
    })
